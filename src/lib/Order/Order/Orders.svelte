...
<div class="text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:text-gray-400 dark:border-gray-700 overflow-y-hidden overflow-x-auto">
    <ul class="flex space-x-4 -mb-px  w-full">
        <li>
            <a href="#" on:click|preventDefault={setFilter.bind(this, 'status', undefined)}
        ...
        <li>
            <a href="#" on:click|preventDefault={setFilter.bind(this, 'status', status.id)}
        ...
        <li>
            ...
            <Dropdown bind:open={dropdownOpen}>
                {#each statuses.filter(s => !statusFiltersShown.includes(s.id)) as status}
                <DropdownItem on:click={setFilter.bind(this, 'status', status.id)}
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        ...
                        <td class="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                            ...
                            >
                                <span class="h-1.5 w-1.5 rounded-full" style={`background-color: ${getStatus(order.status).color}`} />

                                <h2 style={`color: ${getStatus(order.status).color}`} class="text-sm font-normal">{getStatus(order.status).label}</h2>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-4 text-sm whitespace-nowrap">
                            <div class="flex items-center gap-x-6">
                                ...
                                <button title="Delete Order" on:click={deleteOrder.bind(this, order.uuid)}
                                        class="text-gray-500 transition-colors duration-200 dark:hover:text-red-500 dark:text-gray-300 hover:text-red-500 focus:outline-none"
                                >
                                    <svg
                                            xmlns="http://www.w3.org/2000/svg"
