<script lang="ts">
    ...
    import {type IPaymentMethod, PaymentMethodService} from "./services/payment-method.service";
    import {
    ...
    import Edit from './edit-method.svelte';
    import Add from './add-method.svelte';
    import type {ISalesChannel} from "../../SalesChannels/services/sales-channels.service";

    let paymentMethods: IPagination<IPaymentMethod> = {data: []} as IPagination<IPaymentMethod>;
    ...
    async function search() {
        ...
        paymentMethods = await service.find({}, ['*']);

    }

    $: filteredItems = paymentMethods.data.filter((item) => item.title.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1);

    async function edit(item: IPaymentMethod) {
        selectedItem = item;
        showEditModal = true;
    }

    async function onAddNew(item: IEvent<IPaymentMethod>) {
        showAddModal = false;
        selectedItem = item;
        showEditModal = true;
    }
...
<Modal size="xl" bind:open={showEditModal} title={`Editing ${selectedItem?.title}`}>
    <Edit uuid={selectedItem.uuid} on:save={onSaved} />
</Modal>

<Modal size="xl" bind:open={showAddModal} title={`Add Payment Method`}>
    <Add on:save={onAddNew} />
</Modal>
...
<TableSearch placeholder="Search by name" hoverable={true} bind:inputValue={searchTerm} >
    <TableHead>
        ...
        <TableHeadCell>Surcharge</TableHeadCell>
    ...
    <TableBody class="divide-y">
        {#each filteredItems as item}
            <TableBodyRow>
                ...
                <TableBodyCell>{item.surcharge}</TableBodyCell>
                <TableBodyCell>{item.provider.title}</TableBodyCell>
                <TableBodyCell>
