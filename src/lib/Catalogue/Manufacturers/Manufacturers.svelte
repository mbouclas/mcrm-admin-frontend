<script lang="ts">
  ...
  const service = new ManufacturersService();
  ...
  async function search() {
    ...
    loading = true;
    items = await service.find(filters, ['manufacturerCategory', 'variants']);
...
<div class="max-w-screen-xl">
  <div class="max-w-screen-sm">
    <h2 class="mb-4 text-xl lg:text-2xl tracking-tight font-extrabold text-gray-900 dark:text-white outline-none">
      <span class="text-blue-400"> {items.total}</span> Manufacturers
    </h2>
  </div>
</div>

<div class="flex items-center space-x-4">
  <button on:click={() => navigate('/catalogue/manufacturers/new')} class="bg-green-500 rounded p-2"
