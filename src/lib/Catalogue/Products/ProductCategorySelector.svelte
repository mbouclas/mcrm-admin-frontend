...
<h3 class="text-xl py-4 font-semibold tracking-tight text-gray-900 dark:text-white">{label} ( {model.length} selected )</h3>
<div class="w-full p-4 max-h-[500px] overflow-y-auto  bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">

    {#if loading}
        <Loading />
    {:else}
    <TreeView tree={items} on:selection={onCategorySelection} bind:model={model} title="Categories" addIdsOnly={false} />
        {/if}
</div>
