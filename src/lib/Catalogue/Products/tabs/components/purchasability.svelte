<script lang="ts">
    ...
    let ready = false;
    onMount(() => {
        if (typeof model['purchasability'] !== 'object') {
            model['purchasability'] = setupModelFromFields({}, fields[0].fields);
        }
...
{#if ready}
    <CustomFields fields={fields[0].fields} let:field={field}  >
        <div class="my-4">
            <FieldRenderer {field} bind:errors={errors} bind:model={model['purchasability'][field['varName']]} outerClass={''} useHeadingsForNestedFields={false} />
        </div>
    </CustomFields>
{/if}
