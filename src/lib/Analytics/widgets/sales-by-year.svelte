<script lang="ts">
    ...
    async function search() {
        stats = await new OrderStatsService().load('salesByYear', dates, 12);

        data = {
            labels: stats.map(row => dayjs(row.saleYear).format('YYYY')),
            datasets: [
                {
                    label: 'Sales',
...
<Widget>
    <svelte:fragment slot="heading">Sales By Year</svelte:fragment>
    <div slot="actions">
        <DateSelector bind:dates={dates} onDateChange={onDateSelect} {id} />
    </div>
    <DateRange bind:dates={dates} />
    {#if ready}
        <Chart {id} {data} bind:triggerUpdate={triggerUpdate} type="bar" />
    {/if}
</Widget>
