<script lang="ts">
    ...
    function composeLabelField(field: string, obj: IGenericObject) {
        ...
        return field.split('.').map(f => obj[f]).join(' ');
    }
...
<button on:click={goBackToParent.bind(this, 0)}>
    ...
    <svg class="w-6 h-6"
            xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M10 6L8.59 7.41L13.17 12l-4.58 4.59L10 18l6-6z"/></svg>
    </span>
...
{#if items}
    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-auto">
        ...
        <tr>
            ...
            <th scope="col"
                class="py-3.5 px-4 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">{label.label}
            </th>
                {/each}
            {#if config.isTree}
                <th scope="col"
...
<td class="py-3.5 px-4 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400 w-12">
    {#if selectMode === 'multiple'}
        <input type="checkbox" on:change={selectItem.bind(this, item)} value={item}
        checked={selectedItems.includes(item)}/>
    {:else if (selectMode === 'single')}
        <button on:click={selectItem.bind(this, item)} class="text-gray-500 dark:text-gray-400 hover:!text-blue-700 hover:bg-gray-500 hover:rounded p-2">
            <svg class="w-5 h-5"
                    xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
        </button>
    {/if}
            </td>
            {#each labels as label}
            <td
                class="py-3.5 px-4 text-sm font-normal text-left rtl:text-right text-gray-500 dark:text-gray-400">{composeLabelField(label.field, item)}
            </td>
                {/each}
            ...
            {#if config.isTree}
                ...
                <button on:click={showChildren.bind(this, item)}>
                    <svg class="h-5 w-5"
                            xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="m19 15l-6 6l-1.42-1.42L15.17 16H4V4h2v10h9.17l-3.59-3.58L13 9l6 6z"/></svg>
                </button>
                    {/if}
                </td>
            {/if}
