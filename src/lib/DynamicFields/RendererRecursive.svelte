      ...
      if (rule.type === 'field') {
        ...
        for (let i = 0; i < parsedValues.length; i++) {
          ...
          if (!isNaN(Number(secondValue))) {
            const isValid = compare(firstValue, specialChar, parseInt(secondValue));
...
{#each sortedFields as field}
      ...
      {#if field.isCollection}
        {#each model[field.varName] as subModel}
          <div class="p-3 bg-blue-500 bg-opacity-10">
            <svelte:self {itemId} {module} bind:model={subModel} {rootModel} fields={field.fields} />
          </div>
        {/each}
      {:else}
        <div class="p-3 bg-blue-500 bg-opacity-10">
          <svelte:self {itemId} {module} bind:model={model[field.varName]} {rootModel} fields={field.fields} />
        </div>
      {/if}
    </div>
  {/if}
  ...
  {#if field.type === 'text'}
    {#if field.ui && field.ui.component === 'DropDown'}
      <DropDown {field} bind:model={model[field.varName]} onChange={onModelChange} />
    {:else if isValidForEdit(field, rootModel || model)}
      <TextInput {field} bind:model={model[field.varName]} onChange={onModelChange} />
    {:else}
      <Text {field} bind:model={model[field.varName]} onChange={onModelChange} />
    {/if}
  {/if}
  ...
  {#if field.type === 'number'}
    {#if field.ui && field.ui.component === 'DropDown'}
      <DropDown {field} bind:model={model[field.varName]} onChange={onModelChange} />
    {:else if isValidForEdit(field, rootModel || model)}
      <NumberInput {field} bind:model={model[field.varName]} onChange={onModelChange} />
    {:else}
      <Text {field} bind:model={model[field.varName]} onChange={onModelChange} />
    {/if}
  {/if}
  ...
  {#if field.type === 'richText'}
    <RichText {field} bind:model={model[field.varName]} onChange={onModelChange} />
  {/if}

  {#if field.type === 'date'}
    <DateInput {field} bind:model={model[field.varName]} onChange={onModelChange} />
  {/if}
  ...
  {#if field.type === 'group'}
    ...
    <Fields fields={field.fields} bind:model={model[field.varName]} {module} {itemId} />
  {/if}
{/each}
